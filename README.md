...
- direct Nubra websocket feeds decoded through repo-local protobuf definitions
...
For the local TradingView webhook / tunnel flow:

- users should install `cloudflared` on their laptops before using the in-app tunnel controls
- the backend expects `CLOUDFLARED_PATH` to point to a working local `cloudflared` binary
...
- Node.js + npm
- `cloudflared` installed locally if you want the in-app webhook / tunnel flow to work on your laptop
- access to Nubra APIs
- valid Nubra login credentials for real auth / backtest usage

Important:

- this repo does **not** require `nubra_python_sdk`
- Nubra connectivity is handled through REST APIs plus direct websocket/protobuf handling already included in this repo
...
- no separate Nubra Python SDK installation requirement
- local webhook/tunnel controls to expect `cloudflared` to be installed on the laptop
