...
from app.services.strategy_backtester import ParsedStrategy, _run_instrument, run_backtest
...
def build_strategy(
    *,
    ...
    allocation_mode: str = "split_total",
    initial_capital: float = 10000.0,
    capital_per_instrument: float = 10000.0,
    capital_by_instrument: dict[str, float] | None = None,
) -> ParsedStrategy:
    resolved_capital_by_instrument = capital_by_instrument or {"TEST": capital_per_instrument}
    return ParsedStrategy(
        ...
        capital_by_instrument=resolved_capital_by_instrument,
...
class StrategyBacktesterTests(unittest.TestCase):
    ...
    def test_run_instrument_sets_warning_when_no_entry_signals_fire(self) -> None:
        price = indicator_expr("PRICE", source="close")
        entry = Condition(lhs=price, operator="greater_than", rhs=NumberOperand(1000.0))
        strategy = build_strategy(entry_side="BUY", entry_conditions=entry)
        df = make_df(
            [
                ("2024-01-01 15:30", 99.0, 101.0, 98.0, 100.0, 1000.0),
                ("2024-01-02 15:30", 100.0, 102.0, 99.0, 101.0, 1000.0),
            ]
        )
        ...
        self.assertIn("No entry signals", result.warning or "")

    def test_run_backtest_raises_when_portfolio_has_zero_trades(self) -> None:
        price = indicator_expr("PRICE", source="close")
        entry = Condition(lhs=price, operator="greater_than", rhs=NumberOperand(1000.0))
        strategy = build_strategy(entry_side="BUY", entry_conditions=entry)
        df = make_df(
        ...
        enriched = inject_indicator_columns(df, [price])

        with patch("app.services.strategy_backtester.fetch_with_warmup", return_value=(df, 0, 1)), patch(
            "app.services.strategy_backtester.inject_indicator_columns",
        ...
        self.assertIn("No entry signals", str(context.exception.detail))

    def test_run_instrument_uses_symbol_specific_capital(self) -> None:
        price = indicator_expr("PRICE", source="close")
        entry = Condition(lhs=price, operator="less_than", rhs=NumberOperand(100.1))
        strategy = build_strategy(
            ...
            capital_per_instrument=7500.0,
            capital_by_instrument={"TEST": 15000.0},
        ...
        self.assertEqual(result.metrics.starting_capital, 15000.0)
