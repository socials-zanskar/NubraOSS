...
def _add_local_sdk_path() -> None:
    current = Path(__file__).resolve()
    candidates: list[Path] = []
    for parent in current.parents:
        candidates.extend(
            [
                parent / "nubra-mcp-server" / ".venv" / "Lib" / "site-packages",
                parent / "nubra-mcp-server" / "nubra-mcp-server" / ".venv" / "Lib" / "site-packages",
                parent / "active" / "nubra-mcp-server" / "nubra-mcp-server" / ".venv" / "Lib" / "site-packages",
            ]
        )
    for candidate in candidates:
        if (candidate / "nubra_python_sdk").exists():
...
class VolumeQuoteLiveSession:
    ...
    def _dispatch_orderbook(self, raw: bytes) -> None:
        ...
        try:
            envelope = nubrafrontend_pb2.GenericData()
