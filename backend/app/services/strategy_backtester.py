...
def parse_strategy(payload: dict[str, Any]) -> ParsedStrategy:
    ...
    if allocation_mode not in {"split_total", "per_stock", "custom"}:
        raise HTTPException(status_code=400, detail="capital_allocation.mode must be one of: split_total, per_stock, custom.")
    ...
    if allocation_mode == "split_total":
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail="capital_allocation.per_stock_capital must be numeric.") from exc
        if per_stock_capital <= 0:
            raise HTTPException(status_code=400, detail="capital_allocation.per_stock_capital must be positive.")
        capital_per_instrument = per_stock_capital
        initial_capital = capital_per_instrument * len(instruments)
        capital_by_instrument = {symbol: capital_per_instrument for symbol in instruments}
    else:
        custom_capital_map_raw = capital_allocation_payload.get("custom_capital_map")
        if not isinstance(custom_capital_map_raw, dict) or not custom_capital_map_raw:
            raise HTTPException(status_code=400, detail="capital_allocation.custom_capital_map is required when capital_allocation.mode is 'custom'.")
        capital_by_instrument = {}
        for symbol in instruments:
            raw_value = custom_capital_map_raw.get(symbol)
            try:
                parsed_value = float(raw_value)
            except (TypeError, ValueError) as exc:
                raise HTTPException(status_code=400, detail=f"Custom capital for {symbol} must be numeric.") from exc
            if parsed_value <= 0:
                raise HTTPException(status_code=400, detail=f"Custom capital for {symbol} must be positive.")
...
def _run_instrument(
    ...
    symbol_capital = strategy.capital_by_instrument.get(symbol, strategy.capital_per_instrument)
    ...
    if executable.empty:
        empty_metrics = _empty_metrics(symbol_capital)
    ...
    if total_trades == 0:
        if entry_signal_hits == 0:
            warning = (
                f"No entry signals were generated for {symbol} in the requested range. "
                "Your entry conditions may be too restrictive for this instrument, interval, or date window."
            )
        elif blocked_entry_signals == entry_signal_hits:
            warning = (
                f"Entry signals were generated for {symbol}, but all of them fell outside the allowed session window. "
                "Adjust holding type, chart interval, or session time filters."
            )
        else:
            warning = (
                f"Entry conditions triggered for {symbol}, but no executable trades were created. "
                "Review session timing, execution style, and rule alignment."
            )
...
def run_backtest(
    *,
    ...
    if total_trades == 0:
        warning_messages = [result.warning for result in instrument_results if result.warning]
        detail = warning_messages[0] if warning_messages else (
            "Backtest produced zero trades in the requested range. "
            "Please review the entry and exit conditions, date window, and interval."
        )
