...
- direct websocket/protobuf decoding for live quote streaming is built into the repo
...
### Local websocket protobuf support

The live quote / scalper websocket paths no longer require `nubra_python_sdk`.

They use the repo-local protobuf message definitions directly, so there is no
extra Nubra SDK installation requirement on the server for those features.
...
7. Is direct websocket/protobuf connectivity to Nubra allowed and working in this environment?
