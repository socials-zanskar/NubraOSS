...
export function useAutomation(params: UseAutomationParams): UseAutomationReturn {
  ...
  useEffect(() => {
    ...
    addLog(makeLog('info', `Signal: ${actionSide} ${optionLeg} (${instrumentDisplayName}) × ${lots} lots @ LTP ${ltpPrice ?? '--'}`))
    ...
    void (async () => {
      try {
        ...
        addLog(
          makeLog(
            'success',
            `Order placed: ${actionSide} ${optionLeg} × ${lots} lots. ID ${orderId} ${orderStatus}`.trim(),
          ),
        )
      } catch (err) {
        addLog(
