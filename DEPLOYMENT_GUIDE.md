...
### 3.7 Live volume quotes have an extra dependency

The `/ws/volume-quotes` feature depends on `nubra_python_sdk` protobufs being
...
- If `nubra_python_sdk` is missing, the backend degrades gracefully and returns
  an error for `/ws/volume-quotes`.
...
- `nubra_python_sdk` available on the server if you want `/ws/volume-quotes`
  to work fully
...
## 13. Optional `nubra_python_sdk` Support

The volume quotes websocket feature expects Nubra protobufs from
`nubra_python_sdk` on the server.

Current behavior:

- scalper live has fallback protobuf definitions
- volume quotes do not fully fall back in the same way

If the SDK is missing:

- `/ws/volume-quotes` returns an error message instead of live quote streaming

If you need that feature in production:

- make sure `nubra_python_sdk` is installed or otherwise importable
- verify the backend process can load it
